...
//   int height = features.size(2);
  ...
  ROIPoolForwardLaucher(features, rois, spatial_scale, channels, width,
                        num_rois, pooled_width, output, argmax);
...
int roi_pooling_backward_cuda(at::Tensor top_grad, at::Tensor rois,
  ...
  int pooled_width = top_grad.size(2);
